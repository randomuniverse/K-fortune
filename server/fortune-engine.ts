...
export async function generateYearlyFortune(data: {
  ...
  const individualPrompt = `
당신은 ${data.year}년(병오년, 붉은 말의 해)의 운세를 분석하는 동양 운세 전문가이자 데이터 분석가입니다.
사용자의 사주, 자미두수, 별자리 데이터를 ${data.year}년의 기운(병오년: 화(火)의 기운이 강함)과 대조하여,
카테고리별로 구체적인 전략을 제시하세요.

**[절대 금지 사항]**
1. 소설/감성적 묘사 금지. 데이터 기반 논리적 분석만.
2. "절망", "파멸" 같은 부정적 단어 금지.
3. 추상적 조언 금지. 구체적 시기와 행동을 명시.

**[분석 가이드라인]**

1. **overallSummary (${data.year}년 총평)**:
   - 한 해를 관통하는 핵심 키워드와 전체적인 분위기를 3~4문장으로 요약.
...
2. **businessFortune (사업/재물운 & 월별 전략)**:
   ...
   - **월별 흐름:** 1년 중 가장 운이 좋은 달(Best)과 조심해야 할 달(Worst)을 콕 집어 근거와 함께.
