...
2. **pastInference (운명의 추적, 800자 이상, 10~15문장)**:
   ...
   - **모든 특수살을 빠짐없이 언급**하되, 각 살이 무엇인지 쉬운 비유로 먼저 설명한 뒤, **살 간의 화학적 결합(시너지/충돌)**을 분석하세요.
...
3. **currentState (현재의 딜레마, 300자 이상, 5~7문장)**:
   - 현재의 갈등을 인과관계로 풀이하세요.
   - **pastInference에서 분석한 '기운의 결합'이 만드는 딜레마**를 여기서 구체적으로 다루세요.
...
4. **bottleneck (결정적 병목, 300자 이상, 4~6문장)**:
   ...
   - **pastInference에서 분석한 '기운의 화학적 결합'이 만드는 구체적 딜레마를 여기서 다루세요.**
...
5. **solution (가디언의 처방, 300자 이상, 5~7문장)**:
   ...
   - **bottleneck에서 제시한 딜레마에 대한 구체적 해결책**을 제시하세요. bottleneck과 solution이 논리적으로 연결되어야 합니다.
...
**[JSON 출력 형식]**
{
  ...
  "pastInference": "모든 특수살을 빠짐없이 설명하고, 살 간의 화학적 결합을 분석한 스토리텔링 (공백 포함 800자 이상, 10~15문장)",
  "currentState": "기운의 결합이 만드는 현재 딜레마를 인과관계로 풀이 (300자 이상)",
  "bottleneck": "화학적 결합에서 오는 병목을 사주 구성에서 찾아 지적 (300자 이상)",
  "solution": "bottleneck의 딜레마에 대한 구체적 해결책, 기질을 역이용한 행동 지침 (300자 이상)",
...
2. 짧은 글 절대 금지. pastInference는 800자 이상(10~15문장), 나머지 섹션은 각 300자 이상(5~7문장). 원본보다 더 길고 풍성하게 종합해야 합니다.
