...
를 알려주세요.`;
  ...
  const [saju1, saju2, saju3, zodiac1, zodiac2, zodiac3, num1, num2, num3] = await Promise.all([
    ...
    generateWithRetry(sajuSystemPrompt, sajuUserPrompt, "사주3"),
    generateWithRetry(zodiacSystemPrompt, zodiacUserPrompt, "별자리1"),
    generateWithRetry(zodiacSystemPrompt, zodiacUserPrompt, "별자리2"),
    generateWithRetry(zodiacSystemPrompt, zodiacUserPrompt, "별자리3"),
    generateWithRetry(numerologySystemPrompt, numerologyUserPrompt, "수비학1"),
    generateWithRetry(numerologySystemPrompt, numerologyUserPrompt, "수비학2"),
    generateWithRetry(numerologySystemPrompt, numerologyUserPrompt, "수비학3"),
  ]);
  ...
  if (sajuResults.length < 2 || zodiacResults.length < 2) {
    throw new Error("운세 생성에 실패했습니다. 교차 검증 결과가 부족합니다.");
  }
...
반드시 JSON 형식으로만 응답하세요:
{
  ...
  const synthesisRaw = await generateWithRetry(
    "당신은 운세 교차 검증 전문가입니다. 이모지를 절대 사용하지 마세요. 반드시 JSON 형식으로만 응답하세요.",
    synthesizePrompt,
    "합성"
  );
