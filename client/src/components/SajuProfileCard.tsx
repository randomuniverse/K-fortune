...
import { Flame, Droplets, Mountain, Wind, Gem, User, Sparkles, Brain, Shield, Target, Zap, Clock, MapPin, Palette, UtensilsCrossed, TrendingUp, AlertTriangle, Star, Moon } from "lucide-react";
    ...
    <Section icon={Star} title="자미두수: 내 영혼의 별" delay={0.25}>
      <div className="bg-gradient-to-r from-purple-500/10 to-indigo-500/10 border border-purple-500/20 rounded-xl p-4">
          ...
          {ziwei.stars.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {ziwei.stars.map((star, i) => (
                <span key={i} className="px-2 py-1 bg-purple-500/20 rounded text-xs text-purple-200 font-bold border border-purple-500/30">
                  {star.name}
                </span>
              ))}
            </div>
          ) : (
             <span className="text-xs text-white/60">명무정성 (별이 없는 명궁)</span>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          {ziwei.stars.length === 0 && (
             <p className="text-xs text-white/70">명궁에 주성이 없습니다. 이는 대궁(천이궁)의 영향을 많이 받으며, 환경에 따라 변화무쌍하게 적응하는 유연한 운명을 뜻합니다.</p>
          )}
        </div>
